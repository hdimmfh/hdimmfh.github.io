...
When each kernel performs a large amount of work, launch overhead may be relatively small. When a workload contains many short kernels, however, the CPU may struggle to submit work quickly enough to keep the GPU busy.
...
CUDA Graphs are particularly effective when:

* the same GPU workload runs repeatedly,
* kernel execution times are short,
* many kernels run in each iteration,
* CPU launch overhead limits GPU utilization,
* tensor shapes and execution paths remain stable.

They may not apply when:

* tensor shapes continually change,
* control flow changes between iterations,
* inputs are mutated in unsupported ways,
* the workload includes operations that cannot be captured,
* stable memory addresses cannot be maintained.
...
The full relationship can be reduced to one line.

```text
TorchInductor optimizes the graph
    ↓
Triton generates GPU kernels
    ↓
CUDA Graph replays the GPU workload
```
...
> TorchInductor generates or selects GPU operations, including Triton kernels, and PyTorch can capture those operations into a CUDA Graph for repeated execution.

Triton does not create the CUDA Graph itself, and the graph is not limited to Triton kernels.
